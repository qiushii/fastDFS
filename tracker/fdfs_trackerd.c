...
int main(int argc, char *argv[])
{
	...
	if ((result=get_base_path_from_conf_file(conf_filename,
		g_fdfs_base_path, sizeof(g_fdfs_base_path))) != 0)		// 获取 比如base_path = /home/fastdfs/tracker
	{
		log_destroy();
		return result;
	}
	...
	snprintf(pidFilename, sizeof(pidFilename),
		"%s/data/fdfs_trackerd.pid", g_fdfs_base_path);	// 比如 /home/fastdfs/tracker/data/fdfs_trackerd.pid
    action = argc >= 3 ? argv[2] : "start";		// 默认是start
	...
	if ((result=tracker_load_from_conf_file(conf_filename, \		// 加载绑定地址
