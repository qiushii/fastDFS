		...
		struct fast_task_info *pTask, const byte cmd)
{
	...
	memcpy(p, pGroup->group_name, FDFS_GROUP_NAME_MAX_LEN);		// group 组名
	p += FDFS_GROUP_NAME_MAX_LEN;
	strcpy(p, fdfs_get_ipaddr_by_peer_ip(						// storage ip地址
                &ppStoreServers[0]->ip_addrs, pTask->client_ip));
	p += IP_ADDRESS_SIZE - 1;
	long2buff(pGroup->storage_port, p);							// 对应的端口
	p += FDFS_PROTO_PKG_LEN_SIZE;
		...
		struct fast_task_info *pTask, char cmd)
{
	...
	if (cmd == TRACKER_PROTO_CMD_SERVICE_QUERY_STORE_WITH_GROUP_ONE
	 || cmd == TRACKER_PROTO_CMD_SERVICE_QUERY_STORE_WITH_GROUP_ALL)
	{
		group_name = pTask->data + sizeof(TrackerHeader); // 组名
		group_name[FDFS_GROUP_NAME_MAX_LEN] = '\0';
	...
	else if (g_groups.store_lookup == FDFS_STORE_LOOKUP_ROUND_ROBIN		// 轮询
		||g_groups.store_lookup==FDFS_STORE_LOOKUP_LOAD_BALANCE)		// 负载均衡
	{
...
int tracker_deal_task(struct fast_task_info *pTask)
{
	...
	pHeader = (TrackerHeader *)pTask->data;logNotice("cmd:%d, len:%ld\n", pHeader->cmd, *((long *)pHeader->pkg_len));
	switch(pHeader->cmd)
	{
		case TRACKER_PROTO_CMD_STORAGE_BEAT: // 心跳协议
	...
	pHeader = (TrackerHeader *)pTask->data;		// 响应数据起始地址(header起始)
