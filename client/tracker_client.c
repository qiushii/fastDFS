		...
		int *store_path_index)
{
	...
	memset(&header, 0, sizeof(header));		// 因为没有带body, 所以pkg_len保持为0
	header.cmd = TRACKER_PROTO_CMD_SERVICE_QUERY_STORE_WITHOUT_GROUP_ONE;		// 向tracker server请求上传
	if ((result=tcpsenddata_nb(conn->sock, &header, \
			sizeof(header), g_fdfs_network_timeout)) != 0)
	{
		logError("file: "__FILE__", line: %d, " \		// 发送请求失败
			"send data to tracker server %s:%d fail, " \
			"errno: %d, error info: %s", __LINE__, \
			pTrackerServer->ip_addr, \
			pTrackerServer->port, \
	...
	{
		pInBuff = in_buff;
		result = fdfs_recv_response(conn, \				// 发送请求成功，读取响应信息
				&pInBuff, sizeof(in_buff), &in_bytes); // pInBuff 不包括header长度
        if (result != 0)
        {
            logError("file: "__FILE__", line: %d, "
                    "fdfs_recv_response fail, result: %d",
	...
	memcpy(group_name, in_buff, FDFS_GROUP_NAME_MAX_LEN);		// group名字 16字节
	*(group_name + FDFS_GROUP_NAME_MAX_LEN) = '\0';
	memcpy(pStorageServer->ip_addr, in_buff + \					// ip地址，16字节
			FDFS_GROUP_NAME_MAX_LEN, IP_ADDRESS_SIZE-1);
	pStorageServer->port = (int)buff2long(in_buff + \			// 端口
				FDFS_GROUP_NAME_MAX_LEN + IP_ADDRESS_SIZE - 1);
