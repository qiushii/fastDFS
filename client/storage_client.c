		...
		char **file_buff, void *arg, int64_t *file_size)
{
	...
	*file_size = 0;
	if ((result=storage_get_read_connection(pTrackerServer, \
		...
		&storageServer, &new_connection)) != 0)			// 发起请求连接
	{
		return result;
	}
	...
	**/
	...
	{
		if ((result=fdfs_recv_header(pStorageServer, \		// 请求接收header
			&in_bytes)) != 0)
		{
            logError("file: "__FILE__", line: %d, "
		...
		if ((result=tcprecvfile(pStorageServer->sock, \		// 接收文件
				*file_buff, in_bytes, 0, \
	...
	char *group_name, char *remote_filename)
{
	...
	*/
	...
	{
		*p++ = (char)new_store_path;	// storage_index 1字节
	}

	long2buff(file_size, p);			// 文件长度       8字节
	p += FDFS_PROTO_PKG_LEN_SIZE;
	...
	{
		memset(p, 0, FDFS_FILE_EXT_NAME_MAX_LEN);	// 扩展名 6字节
	}
	...
	long2buff((p - out_buff) + file_size - sizeof(TrackerHeader), \
		pHeader->pkg_len);						// 这个包长度包括： 
	pHeader->cmd = cmd;
	pHeader->status = 0;

	if ((result=tcpsenddata_nb(pStorageServer->sock, out_buff, \
		p - out_buff, g_fdfs_network_timeout)) != 0)		// 请求上传文件
	{
		logError("file: "__FILE__", line: %d, " \
			"send data to storage server %s:%d fail, " \
			"errno: %d, error info: %s", __LINE__, \
	...
	{
		if ((result=tcpsendfile(pStorageServer->sock, file_buff, \	// 这里才是真正发送文件
	...
	pInBuff = in_buff;
	if ((result=fdfs_recv_response(pStorageServer, \			// 获取响应
		&pInBuff, sizeof(in_buff), &in_bytes)) != 0)
	{
		logError("file: "__FILE__", line: %d, "
                "fdfs_recv_response fail, result: %d",
