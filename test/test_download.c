...
int main(int argc, char **argv)
{
	...
	{
		conf_filename = "/etc/fdfs/client.conf";	// 使用默认即可
	}

	if ((result = load_file_ids()) != 0)		// 本质而言，就是获取file id和filetype
	{
		return result;
	}
    ...
    if ((result=my_daemon_init()) != 0)		// 后台运行
	{
		return result;
	}
	...
	*/
	...
	total_count = 0;
	success_count = 0;
	while (time(NULL) - start_time < TOTAL_SECONDS)   // 总共的测试的时间
	{
		file_index = (int)(file_count * ((double)rand() / RAND_MAX));		// 随机读取大小文件
		...
		*storage_ip = '\0';
		result = download_file(file_entries[file_index].file_id, &file_size, storage_ip);	// 下载文件
		...
		{
			success_count++;
			files[file_type].success_count++;	// 下载成功
		}
		else //fail
		{
...
static int load_file_ids()
{
	...
	sprintf(filename, "upload/%s.%d", FILENAME_FILE_ID, proccess_index / 2);		// 不是每个file.id都读取
	if ((result=getFileContent(filename, &file_buff, &file_size)) != 0)			// 加载文件内容
	{
	...
	while (*p != '\0')
	{
		if (*p == '\n')
		{
			nLineCount++;		// 计算file.id行数 
		}

		p++;
	}

	file_count = nLineCount / 2;		// 又只读取二分之一
	if (file_count == 0)
	{
		printf("file: "__FILE__", line: %d, " 
	...
	while (i < file_count)
	{
		if (*p == '\n')
		{
			...
			*pEnd = '\0';
			...
			if (file_entries[i].file_id == NULL)			// 加载文件信息
			{
				printf("file: "__FILE__", line: %d, " 
					"malloc %d bytes fail\n", __LINE__, \
					(int)strlen(pFind) + 1);
