		...
		char *bind_addr, const int addr_size)
{
	...
	do
	{
		...
		*/
		...
		if (g_check_file_duplicate)		// 如果校验文件，确保只存储一个内容同样的文件
		{
			char *pKeyNamespace;
			char *pFileSignatureMethod;

			pFileSignatureMethod = iniGetStrValue(NULL, \    // file_signature_method 默认是hash, 也可设置为md5
			...
			memcpy(g_key_namespace, pKeyNamespace, g_namespace_len);	// 存储key
