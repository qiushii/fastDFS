	...
	char proto_cmd)
{
	...
	else
	{
		...
		sprintf(full_filename, "%s/data/%s", \
			...
			pRecord->true_filename);  // 完整path路径，比如/home/fastdfs/storage_group1_23000/data/00/00/eBuDxWCvhiyARqhmACV58jYX3JA.tar.gz
	}
	...
	do
	{
		...
		if((result=tcpsenddata_nb(pStorageServer->sock, out_buff, \
			p - out_buff, g_fdfs_network_timeout)) != 0) // 这里只是发送信令协议
		{
			logError("file: "__FILE__", line: %d, " \
		...
		if (need_sync_file && (stat_buf.st_size > 0) && \
			((result=tcpsendfile_ex(pStorageServer->sock, \		// 真正发送文件
		...
		*/
			...
			pStorage->status == FDFS_STORAGE_STATUS_SYNCING))
		{
			...
			else
			{
				...
				&storage_server, &record)) != 0)		// 往其他storage里同步数据
			{
				logDebug("file: "__FILE__", line: %d, " \
